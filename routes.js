...
Object.assign(I18N.en, { vote: 'Vote', voted: 'Voted', selectCategory: 'Select Category', watchVideo: 'Watch Video', videoUnavailable: 'Video Unavailable', fileTooLarge: 'File size too large (Max 5MB)', candidateFileTooLarge: 'File exceeds maximum limit of 25MB.', seeOriginalFull: 'See Original Full', highQualityPhoto: 'High Quality Photo', closeImage: 'Close Image', enableHighQualityImages: 'Enable High-Quality Images for Participants', unsupportedFileFormat: 'Unsupported file format' });
Object.assign(I18N.ru, { vote: 'Голосовать', voted: 'Проголосовано', selectCategory: 'Выберите категорию', watchVideo: 'Смотреть видео', videoUnavailable: 'Видео недоступно', fileTooLarge: 'Размер файла слишком большой (макс. 5 МБ)', candidateFileTooLarge: 'Файл превышает максимальный размер 25 МБ.', seeOriginalFull: 'Открыть оригинал', highQualityPhoto: 'Фото высокого качества', closeImage: 'Закрыть изображение', enableHighQualityImages: 'Включить изображения высокого качества для участников', unsupportedFileFormat: 'Неподдерживаемый формат файла' });
Object.assign(I18N.ro, { vote: 'Votează', voted: 'Votat', selectCategory: 'Selectează categoria', watchVideo: 'Vezi video', videoUnavailable: 'Videoclip indisponibil', fileTooLarge: 'Fișierul este prea mare (max. 5 MB)', candidateFileTooLarge: 'Fișierul depășește limita maximă de 25 MB.', seeOriginalFull: 'Vezi originalul complet', highQualityPhoto: 'Fotografie de înaltă calitate', closeImage: 'Închide imaginea', enableHighQualityImages: 'Activează imagini de înaltă calitate pentru participanți', unsupportedFileFormat: 'Format de fișier neacceptat' });
...
const uploadCandidateImage = (req, res, next) => req.app.locals.candidateUpload.single('image')(req, res, next);
...
router.post('/api/telegram-auth', asyncRoute(async (req, res) => {
    ...
    const status = await dbHandler.getTelegramUserStatus(user.telegramId);
    const currentUser = status.data || user;
    return res.json({ success: true, voted: Boolean(status.voted), votedCategories: currentUser.votedCategories || [], allowedPrizes: Boolean(currentUser.allowedPrizes), scheduleNotificationsEnabled: Boolean(currentUser.scheduleNotificationsEnabled), votingStartNotificationsEnabled: Boolean(currentUser.votingStartNotificationsEnabled), user: { id: currentUser.telegramId, username: currentUser.username, languageCode: currentUser.languageCode } });
}));
...
router.post('/admin/graphics/theme', adminMutationGuard, asyncRoute(async (req, res) => {
    await dbHandler.updatePublicTheme(req.body && req.body.publicTheme);
    return res.json({ success: true, redirect: '/admin/graphics' });
}));
...
router.post('/admin/categories/:categoryId/candidates', adminMutationGuard, uploadCandidateImage, asyncRoute(async (req, res) => { const images = await req.app.locals.processCandidateImage(req.file); await dbHandler.createCandidate({ categoryId: req.params.categoryId, name: req.body.name, description: req.body.description, code: req.body.code, videoUrl: participantVideoUrl(req.body.videoUrl), order: req.body.order, ...images }); res.json({ success: true, redirect: categoryPath(req.params.categoryId) }); }));
router.post('/admin/categories/:categoryId/candidates/:candidateId/update', adminMutationGuard, uploadCandidateImage, asyncRoute(async (req, res) => { const payload = { categoryId: req.params.categoryId, name: req.body.name, description: req.body.description, code: req.body.code, videoUrl: participantVideoUrl(req.body.videoUrl), order: req.body.order, ...(await req.app.locals.processCandidateImage(req.file)) }; await dbHandler.updateCandidate(req.params.candidateId, payload); res.json({ success: true, redirect: categoryPath(req.params.categoryId) }); }));
